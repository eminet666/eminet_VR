...
StarrySky.Materials.Atmosphere.atmosphereShader = {
    uniforms: function (isSunShader = false, isMoonShader = false, isMeteringShader = false) {
        ...
        if (isSunShader) {
            uniforms.sunAngularDiameterCos = {
                type: "f",
                value: 1
            };
            uniforms.radiusOfSunPlane = {
                type: "f",
                value: 1
            };
            uniforms.moonRadius = {
            ...
            uniforms.solarEclipseMap = {
                type: "t",
                value: null
            };
            uniforms.moonDiffuseMap = {
                type: "t",
                value: null
            }
        } else if (isMoonShader) {
            uniforms.moonExposure = {
                type: "f",
                value: 1
            };
            uniforms.moonAngularDiameterCos = {
                type: "f",
                value: 1
            };
            uniforms.sunRadius = {
            ...
            uniforms.radiusOfMoonPlane = {
                type: "f",
                value: 1
            };
            uniforms.distanceToEarthsShadowSquared = {
                type: "f",
                value: 1
            };
            uniforms.oneOverNormalizedLunarDiameter = {
                type: "f",
                value: 1
            };
            uniforms.worldMatrix = {
            ...
            uniforms.sunLightDirection = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.earthsShadowPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.moonDiffuseMap = {
            ...
            uniforms.moonNormalMap = {
                type: "t",
                value: null
            };
            uniforms.moonRoughnessMap = {
                type: "t",
                value: null
            };
            uniforms.moonApertureSizeMap = {
                type: "t",
                value: null
            };
            uniforms.moonApertureOrientationMap = {
                type: "t",
                value: null
            }
        }
        if (!isSunShader) {
            uniforms.starHashCubemap = {
                type: "t",
                value: null
            };
            uniforms.dimStarData = {
                type: "t",
                value: null
            };
            uniforms.medStarData = {
                type: "t",
                value: null
            };
            uniforms.brightStarData = {
                type: "t",
                value: null
            };
            uniforms.starColorMap = {
                type: "t",
                value: null
            };
            uniforms.mercuryPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.venusPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.marsPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.jupiterPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.saturnPosition = {
                type: "vec3",
                value: new THREE.Vector3
            };
            uniforms.mercuryBrightness = {
                type: "f",
                value: 0
            };
            uniforms.venusBrightness = {
                type: "f",
                value: 0
            };
            uniforms.marsBrightness = {
                type: "f",
                value: 0
            };
            uniforms.jupiterBrightness = {
                type: "f",
                value: 0
            };
            uniforms.saturnBrightness = {
                type: "f",
                value: 0
            }
        }
        if (!isSunShader && !isMeteringShader) {
            uniforms.starsExposure = {
                type: "f",
                value: -4
            }
        }
        if (isMeteringShader) {
            uniforms.sunLuminosity = {
                type: "f",
                value: 20
            };
            uniforms.moonLuminosity = {
                type: "f",
                value: 1.4
            }
        }
...
StarrySky.Materials.Postprocessing.seperableBlurFilter = {
    ...
    fragmentShader: function (kernalRadius, textureSize) {
        let originalGLSL = ["//Derivative of Unreal Bloom Pass from Three.JS", "//Thanks spidersharma / http://eduperiment.com/", "//", "uniform sampler2D sourceTexture;", "uniform vec2 direction;", "//Based on Luminosity High Pass Shader", "//Originally created by bhouston / http://clara.io/", "void main(){", "vec2 vUv = gl_FragCoord.xy / resolution.xy;", "float weightedSum = $gaussian_pdf_at_x_0;", "vec3 diffuseSum = texture2D(sourceTexture, vUv).rgb * weightedSum;", "//Unrolled for loop (completed in material function)", "$unrolled_for_loop", "gl_FragColor = vec4(abs(diffuseSum/weightedSum), 1.0);", "}"];
        let oneOverSigmaSquared = 1 / kernalRadius * kernalRadius;
        let inverseSigma = 1 / kernalRadius;

        function gaussianPdf(x) {
            return .39894 * Math.exp(-.5 * x * x * oneOverSigmaSquared) * inverseSigma
        }
        let invSize = 1 / textureSize;
        let gaussianPdfAtX0 = gaussianPdf(0);
        let unrolledForLoop = [];
        let w = gaussianPdf(1);
        let invSizeTimesI = invSize;
...
StarrySky.Materials.Autoexposure.meteringSurvey = {
    vertexShader: ["varying vec3 vWorldPosition;", "varying vec2 vUv;", "void main() {", "//Just pass over the texture coordinates", "vUv = uv * 2.0 - 1.0;", "gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);", "}"].join("\n")
};
StarrySky.Materials.Autoexposure.testPass = {
    vertexShader: ["varying vec3 vWorldPosition;", "varying vec2 vUv;", "void main() {", "//Just pass over the texture coordinates", "vUv = uv;", "gl_Position = vec4(position, 1.0);", "}"].join("\n"),
...
StarrySky.DefaultData.fileNames = {
    ...
    starHashCubemap: ["star-dictionary-cubemap-px.png", "star-dictionary-cubemap-nx.png", "star-dictionary-cubemap-py.png", "star-dictionary-cubemap-ny.png", "star-dictionary-cubemap-pz.png", "star-dictionary-cubemap-nz.png"],
    dimStarDataMaps: ["dim-star-data-r-channel.png", "dim-star-data-g-channel.png", "dim-star-data-b-channel.png", "dim-star-data-a-channel.png"],
    medStarDataMaps: ["med-star-data-r-channel.png", "med-star-data-g-channel.png", "med-star-data-b-channel.png", "med-star-data-a-channel.png"],
    brightStarDataMaps: ["bright-star-data-r-channel.png", "bright-star-data-g-channel.png", "bright-star-data-b-channel.png", "bright-star-data-a-channel.png"],
    starColorMap: "star-color-map.png",
    blueNoiseMaps: ["blue-noise-0.bmp", "blue-noise-1.bmp", "blue-noise-2.bmp", "blue-noise-3.bmp", "blue-noise-4.bmp"],
...
class SkyAssetsDir extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.addEventListener("DOMContentLoaded", (function (evt) {
            ...
            const objectProperties = ["moonDiffuseMap", "moonNormalMap", "moonRoughnessMap", "moonApertureSizeMap", "moonApertureOrientationMap", "starHashCubemap", "dimStarMaps", "medStarMaps", "brightStarMaps", "starColorMap", "blueNoiseMaps", "solarEclipseMap"];
            const tagsList = [moonDiffuseMapTags, moonNormalMapTags, moonRoughnessMapTags, moonApertureSizeMapTags, moonApertureOrientationMapTags, starCubemapTags, medStarMapTags, dimStarMapTags, brightStarMapTags, starColorMapTags, blueNoiseMapTags, solarEclipseMapTags];
            const numberOfTagTypes = tagsList.length;
            if (self.hasAttribute("texture-path") && self.getAttribute("texture-path").toLowerCase() !== "false") {
                const singleTextureKeys = ["moonDiffuseMap", "moonNormalMap", "moonRoughnessMap", "moonApertureSizeMap", "moonApertureOrientationMap", "starColorMap", "solarEclipseMap"];
                const multiTextureKeys = ["starHashCubemap", "dimStarDataMaps", "medStarDataMaps", "brightStarDataMaps", "blueNoiseMaps"];
                ...
                const moonTextureKeys = ["moonDiffuseMap", "moonNormalMap", "moonRoughnessMap", "moonApertureSizeMap", "moonApertureOrientationMap"];
...
class SkyAtmosphericParameters extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.addEventListener("DOMContentLoaded", (function (evt) {
            ...
            self.data.mieDirectionalG = clampAndWarn(self.data.mieDirectionalG, -1, 1, "<sky-mie-directional-g>");
            self.data.sunAngularDiameter = clampAndWarn(self.data.sunAngularDiameter, .1, 90, "<sky-sun-angular-diameter>");
            self.data.moonAngularDiameter = clampAndWarn(self.data.moonAngularDiameter, .1, 90, "<sky-moon-angular-diameter>");
            self.skyDataLoaded = true;
            document.dispatchEvent(new Event("Sky-Data-Loaded"))
        }))
    }
...
class SkyLocation extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.addEventListener("DOMContentLoaded", (function (evt) {
            ...
            self.data.latitude = self.data.latitude ? clampAndWarn(self.data.latitude, -90, 90, "<sky-latitude>") : null;
            self.data.longitude = self.data.longitude ? clampAndWarn(self.data.longitude, -180, 180, "<sky-longitude>") : null;
            self.skyDataLoaded = true;
            document.dispatchEvent(new Event("Sky-Data-Loaded"))
        }))
    }
...
(function setupAStarrySkyDefaultTimeData() {
    const now = new Date;
    StarrySky.DefaultData.time = {
        date: [now.getMonth() + 1, now.getDate(), now.getFullYear()].join("/") + " " + [now.getHours(), now.getMinutes(), now.getSeconds()].join(":"),
        utcOffset: 7,
        speed: 1
    }
})();
class SkyTime extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.addEventListener("DOMContentLoaded", (function (evt) {
            ...
            self.data.utcOffset = self.data.utcOffset ? clampAndWarn(self.data.utcOffset, -14, 12, "<sky-utc-offset>") : null;
            self.data.speed = self.data.speed ? clampAndWarn(self.data.speed, 0, 1e3, "<sky-speed>") : null;
            self.skyDataLoaded = true;
            document.dispatchEvent(new Event("Sky-Data-Loaded"))
        }))
    }
...
StarrySky.DefaultData.lighting = {
    ...
    atmosphericPerspectiveDensity: .007,
    shadowCameraSize: 32,
...
class SkyLighting extends HTMLElement {
    ...
    connectedCallback() {
        ...
        document.addEventListener("DOMContentLoaded", (function (evt) {
            ...
            self.data.atmosphericPerspectiveEnabled = self.data.atmosphericPerspectiveDensity > 0;
            ...
            self.data.atmosphericPerspectiveDensity = clampAndWarn(self.data.atmosphericPerspectiveDensity, 0, Infinity, "<sky-atmospheric-perspective-density>");
            self.data.shadowCameraSize = clampAndWarn(self.data.shadowCameraSize, 0, Infinity, "<sky-shadow-camera-size>");
...
StarrySky.LUTlibraries.AtmosphericLUTLibrary = function (data, renderer, scene) {
    ...
    this.atmosphereFunctionsString = materials.atmosphereFunctions.partialFragmentShader(this.scatteringTextureWidth, this.scatteringTextureHeight, this.scatteringTexturePackingWidth, this.scatteringTexturePackingHeight, this.data.skyAtmosphericParameters.mieDirectionalG);
    let atmosphereFunctions = this.atmosphereFunctionsString;
    let transmittanceTexture = transmittanceRenderer.createTexture();
    let transmittanceVar = transmittanceRenderer.addVariable("transmittanceTexture", materials.transmittanceMaterial.fragmentShader(data.skyAtmosphericParameters.numberOfRaySteps, atmosphereFunctions), transmittanceTexture);
    ...
    let singleScatteringRayleighVar = singleScatteringRenderer.addVariable("kthInscatteringRayleigh", materials.singleScatteringMaterial.fragmentShader(data.skyAtmosphericParameters.numberOfRaySteps, this.scatteringTextureWidth, this.scatteringTextureHeight, this.scatteringTexturePackingWidth, this.scatteringTexturePackingHeight, true, atmosphereFunctions), singleScatteringRayleighTexture);
    ...
    let inscatteringRayleighSumVar = scatteringSumRenderer.addVariable("inscatteringRayleighSumTexture", materials.inscatteringSumMaterial.fragmentShader, inscatteringRayleighSumTexture);
    ...
    let inscatteringMieSumVar = scatteringSumRenderer.addVariable("inscatteringMieSumTexture", materials.inscatteringSumMaterial.fragmentShader, inscatteringMieSumTexture);
    ...
    let multipleScatteringMieVar = multipleScatteringRenderer.addVariable("kthInscatteringMie", materials.kthInscatteringMaterial.fragmentShader(data.skyAtmosphericParameters.numberOfRaySteps, this.scatteringTextureWidth, this.scatteringTextureHeight, this.scatteringTexturePackingWidth, this.scatteringTexturePackingHeight, mieGCoefficient, false, atmosphereFunctions), multipleScatteringMieTexture);
    ...
    let multipleScatteringRayleighVar = multipleScatteringRenderer.addVariable("kthInscatteringRayleigh", materials.kthInscatteringMaterial.fragmentShader(data.skyAtmosphericParameters.numberOfRaySteps, this.scatteringTextureWidth, this.scatteringTextureHeight, this.scatteringTexturePackingWidth, this.scatteringTexturePackingHeight, mieGCoefficient, true, atmosphereFunctions), multipleScatteringRayleighTexture);
...
StarrySky.LUTlibraries.StellarLUTLibrary = function (data, renderer, scene) {
    ...
    this.dimStarMapVar = this.dimStarDataRenderer.addVariable("dimStarMapTexture", materials.starDataMap.fragmentShader, this.dimStarMapTexture);
    ...
    this.medStarMapVar = this.medStarDataRenderer.addVariable("medStarMapTexture", materials.starDataMap.fragmentShader, this.medStarMapTexture);
    ...
    this.brightStarMapVar = this.brightStarDataRenderer.addVariable("brightStarMapTexture", materials.starDataMap.fragmentShader, this.brightStarMapTexture);
...
StarrySky.Renderers.AtmosphereRenderer = function (skyDirector) {
    ...
    this.geometry = new THREE.OctahedronBufferGeometry(5e3, 5);
    this.atmosphereMaterial = new THREE.ShaderMaterial({
        ...
        fragmentShader: StarrySky.Materials.Atmosphere.atmosphereShader.fragmentShader(skyDirector.assetManager.data.skyAtmosphericParameters.mieDirectionalG, skyDirector.atmosphereLUTLibrary.scatteringTextureWidth, skyDirector.atmosphereLUTLibrary.scatteringTextureHeight, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingWidth, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingHeight, skyDirector.atmosphereLUTLibrary.atmosphereFunctionsString)
    });
    ...
    this.firstTick = function (t) {
        ...
        self.atmosphereMaterial.uniforms.latitude.value = self.skyDirector.assetManager.data.skyLocationData.latitude * (Math.PI / 180);
...
StarrySky.Renderers.BloomRenderer = function (skyDirector, targetName, threshold) {
    ...
    const blurDirectionX = new THREE.Vector2(1, 0);
    const blurDirectionY = new THREE.Vector2(0, 1);
    ...
    this.highPassFilterVar = this.highPassRenderer.addVariable(`${targetName}.highPassFilter`, materials.highPassFilter.fragmentShader, this.highPassFilterTexture);
    ...
    for (let i = 0; i < 5; ++i) {
        ...
        this.seperableBlurHorizontalVars.push(this.seperableBlurHorizontalRenderers[i].addVariable(`${targetName}.seperableHorizontalBlur.${i}`, materials.seperableBlurFilter.fragmentShader(kernalSize, mipSize), this.seperableBlurHorizontalTextures[i]));
        ...
        this.seperableBlurVerticalVars.push(this.seperableBlurVerticalRenderers[i].addVariable(`${targetName}.seperableVerticalBlur.${i}`, materials.seperableBlurFilter.fragmentShader(kernalSize, mipSize), this.seperableBlurVerticalTextures[i]));
...
StarrySky.Renderers.SunRenderer = function (skyDirector) {
    ...
    const RADIUS_OF_SKY = 5e3;
    const DEG_2_RAD = .017453292519943295;
    const moonAngularRadiusInRadians = skyDirector.assetManager.data.skyAtmosphericParameters.moonAngularDiameter * DEG_2_RAD * .5;
    const baseRadiusOfTheMoon = Math.sin(moonAngularRadiusInRadians);
    this.sunAngularRadiusInRadians = skyDirector.assetManager.data.skyAtmosphericParameters.sunAngularDiameter * DEG_2_RAD * .5;
    const radiusOfSunPlane = RADIUS_OF_SKY * Math.sin(this.sunAngularRadiusInRadians) * 2;
    const diameterOfSunPlane = 2 * radiusOfSunPlane;
    ...
    this.baseSunVar = this.sunRenderer.addVariable("baseSunTexture", StarrySky.Materials.Atmosphere.atmosphereShader.fragmentShader(skyDirector.assetManager.data.skyAtmosphericParameters.mieDirectionalG, skyDirector.atmosphereLUTLibrary.scatteringTextureWidth, skyDirector.atmosphereLUTLibrary.scatteringTextureHeight, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingWidth, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingHeight, skyDirector.atmosphereLUTLibrary.atmosphereFunctionsString, baseSunPartial, false), this.baseSunTexture);
    ...
    this.setBloomStrength(3);
    this.setBloomRadius(.7);
    this.tick = function (t) {
        ...
        self.sunMesh.lookAt(cameraPosition.x, cameraPosition.y, cameraPosition.z);
        ...
        let bloomTest = self.skyDirector.skyState.sun.horizonFade >= .95;
    ...
    this.firstTick = function (t) {
        ...
        self.baseSunVar.material.uniforms.latitude.value = self.skyDirector.assetManager.data.skyLocationData.latitude * (Math.PI / 180);
        self.baseSunVar.material.uniforms.moonLightColor.value = self.skyDirector.skyState.moon.lightingModifier;
        self.combinationPassMaterial.uniforms.bloomEnabled.value = self.skyDirector.skyState.sun.horizonFade >= .95;
        self.combinationPassMaterial.uniforms.bloomEnabled.needsUpdate = true;
...
StarrySky.Renderers.MoonRenderer = function (skyDirector) {
    ...
    const RADIUS_OF_SKY = 5e3;
    const DEG_2_RAD = .017453292519943295;
    const sunAngularRadiusInRadians = skyDirector.assetManager.data.skyAtmosphericParameters.sunAngularDiameter * DEG_2_RAD * .5;
    this.moonAngularRadiusInRadians = skyDirector.assetManager.data.skyAtmosphericParameters.moonAngularDiameter * DEG_2_RAD * .5;
    const radiusOfMoonPlane = RADIUS_OF_SKY * Math.sin(this.moonAngularRadiusInRadians) * 2;
    const diameterOfMoonPlane = 2 * radiusOfMoonPlane;
    ...
    this.baseMoonVar = this.moonRenderer.addVariable("baseMoonTexture", StarrySky.Materials.Atmosphere.atmosphereShader.fragmentShader(skyDirector.assetManager.data.skyAtmosphericParameters.mieDirectionalG, skyDirector.atmosphereLUTLibrary.scatteringTextureWidth, skyDirector.atmosphereLUTLibrary.scatteringTextureHeight, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingWidth, skyDirector.atmosphereLUTLibrary.scatteringTexturePackingHeight, skyDirector.atmosphereLUTLibrary.atmosphereFunctionsString, false, baseMoonPartial), this.baseMoonTexture);
    ...
    this.tick = function (t) {
        ...
        self.moonMesh.lookAt(cameraPosition);
        self.moonMesh.rotateOnWorldAxis(self.parallacticAxis, -skyDirector.skyState.moon.parallacticAngle);
    ...
    this.firstTick = function (t) {
        ...
        if (self.skyDirector.assetManager) {
            ...
            self.baseMoonVar.material.uniforms.latitude.value = self.skyDirector.assetManager.data.skyLocationData.latitude * (Math.PI / 180);
...
StarrySky.Renderers.MeteringSurveyRenderer = function (skyDirector) {
    ...
    this.meteringSurveyVar.material.uniforms.latitude.value = skyDirector.assetManager.data.skyLocationData.latitude * (Math.PI / 180);
    ...
    this.render = function (sunPosition, moonPosition, sunFade, moonFade) {
        ...
        self.meteringSurveyVar.material.uniforms.moonHorizonFade.value = Math.max(1 - sunFade, 0);
...
StarrySky.LightingManager = function (parentComponent) {
    ...
    this.sourceLight = new THREE.DirectionalLight(16777215, 4);
    ...
    shadow.camera.near = 128;
    shadow.camera.far = RADIUS_OF_SKY * 2;
    ...
    this.targetScalar = .5 * totalDistance - lightingData.shadowDrawBehindDistance;
    this.shadowTarget = new THREE.Vector3;
    this.shadowTargetOffset = new THREE.Vector3;
    this.fogColorVector = new THREE.Color;
    this.xAxisHemisphericalLight = new THREE.HemisphereLight(0, 0, 0);
    this.yAxisHemisphericalLight = new THREE.HemisphereLight(0, 0, 1);
    this.zAxisHemisphericalLight = new THREE.HemisphereLight(0, 0, 0);
    ...
    if (lightingData.atmosphericPerspectiveEnabled) {
        this.fog = new THREE.FogExp2(16777215, maxFogDensity);
        parentComponent.scene.fog = this.fog
    }
    ...
    this.tick = function (lightingState) {
        if (lightingData.atmosphericPerspectiveEnabled) {
            ...
            self.fog.density = Math.pow(maxColor, .3) * maxFogDensity;
        ...
        const intensityModifier = Math.min(Math.max(lightingState[24] * 2, 0), .1) / .1;
        self.sourceLight.intensity = lightingState[24] * .5;
        ...
        const indirectLightIntensity = .01 + intensityModifier * .15;
    ...
    this.loadImageAssets = async function (renderer) {
        ...
        (async function createNewMoonTexturePromise(i) {
            ...
            let texturePromise = new Promise((function (resolve, reject) {
                textureLoader.load(StarrySky.assetPaths[moonTextures[i]], (function (texture) {
                    resolve(texture)
                }))
            }));
        ...
        let texturePromise = new Promise((function (resolve, reject) {
            textureLoader.load(StarrySky.assetPaths.starColorMap, (function (texture) {
                resolve(texture)
            }))
        }));
        texturePromise.then((function (texture) {
        ...
        let texturePromise2 = new Promise((function (resolve, reject) {
            loader.load(StarrySky.assetPaths.starHashCubemap, (function (cubemap) {
                resolve(cubemap)
            }))
        }));
        ...
        (async function createNewDimStarTexturePromise(i) {
            ...
            const texturePromise = new Promise((function (resolve, reject) {
                textureLoader.load(StarrySky.assetPaths.dimStarDataMaps[i], (function (texture) {
                    resolve(texture)
                }))
            }));
            texturePromise.then((function (texture) {
        ...
        (async function createNewMedStarTexturePromise(i) {
            ...
            let texturePromise = new Promise((function (resolve, reject) {
                textureLoader.load(StarrySky.assetPaths.medStarDataMaps[i], (function (texture) {
                    resolve(texture)
                }))
            }));
            texturePromise.then((function (texture) {
        ...
        (async function createNewBrightStarTexturePromise(i) {
            ...
            let texturePromise = new Promise((function (resolve, reject) {
                textureLoader.load(StarrySky.assetPaths.brightStarDataMaps[i], (function (texture) {
                    resolve(texture)
                }))
            }));
            texturePromise.then((function (texture) {
        ...
        (async function createNewBlueNoiseTexturePromise(i) {
            ...
            let texturePromise = new Promise((function (resolve, reject) {
                textureLoader.load(StarrySky.assetPaths["blueNoiseMaps"][i], (function (texture) {
                    resolve(texture)
                }))
            }));
            texturePromise.then((function (texture) {
        ...
        let solarEclipseTexturePromise = new Promise((function (resolve, reject) {
            textureLoader.load(StarrySky.assetPaths.solarEclipseMap, (function (texture) {
                resolve(texture)
            }))
        }));
...
StarrySky.SkyDirector = function (parentComponent, webWorkerURI) {
    ...
    this.exposureT = 0;
    this.time = 0;
    this.interpolatedSkyIntensityMagnitude = 1;
    this.ready = false;
    ...
    this.exposureVariables = {
        ...
        sunHorizonFade: 0,
        moonHorizonFade: 0,
        exposureCoefficient0: 0,
        starsExposure: 0,
        moonExposure: 0,
        exposureCoefficientf: 0
    };
    ...
    this.initializeSkyDirectorWebWorker = function () {
        if (self.assetManagerInitialized && self.skyInterpolatorWASMIsReady) {
            self.sunRadius = Math.sin(this.assetManager.data.skyAtmosphericParameters.sunAngularDiameter * DEG_2_RAD * .5);
            self.moonRadius = Math.sin(this.assetManager.data.skyAtmosphericParameters.moonAngularDiameter * DEG_2_RAD * .5);
            self.distanceForSolarEclipse = 2 * Math.SQRT2 * Math.max(self.sunRadius, self.moonRadius);
            ...
            BASE_RADIUS_OF_SUN = self.assetManager.data.skyAtmosphericParameters.sunAngularDiameter * DEG_2_RAD * .5;
    ...
    this.initializeRenderers = function () {
        if (self.assetManagerInitialized && self.skyDirectorWASMIsReady) {
            ...
            const sunRendererTextureSize = Math.floor(self.pixelsPerRadian * sunAngularDiameterInRadians * 2);
            const ceilSRTS = Math.min(parseInt(1 << (32 - Math.clz32(sunRendererTextureSize - 1), 10)), 1024);
            const floorSRTS = ceilSRTS >> 1;
            ...
            const moonRendererTextureSize = Math.floor(self.pixelsPerRadian * moonAngularDiameterInRadians * 2);
            const ceilMRTS = Math.min(parseInt(1 << (32 - Math.clz32(moonRendererTextureSize - 1), 10)), 1024);
            const floorMRTS = ceilMRTS >> 1;
            ...
            self.renderers.bloomRenderer = new StarrySky.Renderers.BloomRenderer(self, "shared", .98);
    ...
    this.tick = function (time, timeDelta) {
        if (parentComponent.initialized) {
            const timeDeltaInSeconds = timeDelta * .001;
            ...
            self.skyState.sun.luminosity = 1e5 * self.astronomicalLinearValues[0] / 1300;
            self.skyState.sun.intensity = 10 * self.astronomicalLinearValues[0] / 1300;
            self.skyState.sun.horizonFade = self.rotatedAstroDependentValues[0];
            self.skyState.sun.scale = self.astronomicalLinearValues[1];
            self.skyState.moon.luminosity = 200 * self.astronomicalLinearValues[2];
            self.skyState.moon.intensity = 500 * self.astronomicalLinearValues[2];
            ...
            if (self.exposureT >= HALF_A_SECOND && self.transferableSkyFinalLightingBuffer.byteLength !== 0) {
                ...
                Module._updateLightingValues(self.interpolatedSkyIntensityMagnitude, self.exposureVariables.exposureCoefficientf, self.dominantLightIsSun0, self.dominantLightIsSunf, self.dominantLightY0, self.dominantLightYf, self.lightingColorValues_ptr, self.lightingColorValues_f_ptr, self.time, self.time + HALF_A_SECOND);
    ...
    this.webAssemblyWorker.addEventListener("message", (function (e) {
        ...
        if (postObject.eventType === self.EVENT_RETURN_LATEST_SKY_STATE) {
            ...
            const twiceTheSinOfSolarRadius = 2 * Math.sin(self.assetManager.data.skyAtmosphericParameters.sunAngularDiameter * DEG_2_RAD * .5);
            Module._setupInterpolators(latitude, twiceTheSinOfSolarRadius, self.sunRadius, self.moonRadius, self.distanceForSolarEclipse, self.astroPositions_0_ptr, self.rotatedAstroPositions_ptr, self.astronomicalLinearValues_0_ptr, self.astronomicalLinearValues_ptr, self.rotatedAstroDepedentValues_ptr);
            Module._updateFinalAstronomicalValues(self.astroPositions_f_ptr, self.astronomicalLinearValues_f_ptr);
            self.finalLSRT = self.finalStateFloat32Array[14];
            self.finalAstronomicalT = self.interpolationT + TWENTY_MINUTES;
            ...
            self.skyState = {
                ...
                moon: {
                    position: new THREE.Vector3,
                    quadOffset: new THREE.Vector3,
                    distanceToEarthsShadowSquared: 0,
                    oneOverNormalizedLunarDiameter: 0,
            ...
            Module._updateLightingValues(postObject.exposureCoefficient0, postObject.exposureCoefficientf, self.dominantLightIsSun0, self.dominantLightIsSunf, postObject.dominantLightY0, postObject.dominantLightYf, self.lightingColorValues_0_ptr, self.lightingColorValues_f_ptr, self.time, self.time + HALF_A_SECOND);
    ...
    this.initializeAutoExposure = async function () {
        ...
        groundColorArray[0] = groundColorRef.red / 255;
        groundColorArray[1] = groundColorRef.green / 255;
        groundColorArray[2] = groundColorRef.blue / 255;
        Module._setSunAndMoonTimeTo(0);
        ...
        }, [transferableSkyIntialLightingBuffer, self.transferableSkyFinalLightingBuffer, self.atmosphereLUTLibrary.transferrableTransmittanceBuffer, groundColorArray.buffer])
    };
    this.updateAutoExposure = function (deltaT) {
        const meteringTextureSize = self.renderers.meteringSurveyRenderer.meteringSurveyTextureSize;
        Module._setSunAndMoonTimeTo(self.interpolationT + 2 * HALF_A_SECOND * self.speed);
        self.exposureVariables.sunPosition.fromArray(self.rotatedAstroPositions.slice(0, 3));
        self.exposureVariables.moonPosition.fromArray(self.rotatedAstroPositions.slice(3, 6));
        self.exposureVariables.sunHorizonFade = self.rotatedAstroDependentValues[0];
        self.exposureVariables.moonHorizonFade = self.rotatedAstroDependentValues[1];
        ...
        self.webAssemblyWorker.postMessage({
            ...
            sunIntensityf: sunIntensityf,
            moonIntensityf: moonIntensityf,
            heightOfCamera: 2 * self.camera.position.y - self.previousCameraHeight,
