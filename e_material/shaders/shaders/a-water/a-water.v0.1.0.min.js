...
AWater.AOcean.Materials.FFTWaves.computeTwiddleIndices = function (N, renderer) {
    ...
    for (let x = 0; x < textureWidth; ++x) {
        twiddleTexture.push([]);
        for (let y = 0; y < textureHeight; ++y) {
            twiddleTexture[x].push([0, 0, 0, 0])
        }
    }
    let butterflySpan = 1;
    let nextButterflySpan = butterflySpan * 2;
    let twoPiOverN = 2 * Math.PI / N;
    for (let y = 0; y < textureHeight; ++y) {
    ...
    let dataTexture = new THREE.DataTexture(new Float32Array(data), actualTextureWidth, textureHeight, THREE.RGBAFormat, /(iPad|iPhone|iPod)/g.test(navigator.userAgent) ? THREE.HalfFloatType : THREE.FloatType, THREE.ClampToEdgeWrapping, THREE.ClampToEdgeWrapping, THREE.NearestFilter, THREE.NearestFilter);
...
AWater.AOcean.Materials.FFTWaves.waveComposerShaderMaterial = {
    uniforms: function (numberOfWaveComponents) {
        return {
            xWavetextures: {
                value: new Array(numberOfWaveComponents)
            },
            yWavetextures: {
                value: new Array(numberOfWaveComponents)
            },
...
AWater.AOcean.Materials.FFTWaves.waveHeightShaderMaterialData = {
    ...
    fragmentShader: ["precision highp float;", "uniform sampler2D combinedWaveHeights;", "uniform float N;", "uniform float waveHeightMultiplier;", "void main(){", "vec2 uv = gl_FragCoord.xy / resolution.xy;", "float outputputColor = waveHeightMultiplier * texture2D(combinedWaveHeights, uv).xyz / (N * N);", "gl_FragColor = vec4(vec3(outputColor), 1.0);", "}"].join("\n")
};
AWater.AOcean.Materials.Ocean.positionPassMaterial = {
    ...
    fragmentShader: ["varying vec3 vWorldPosition;", "void main(){", "//Check if we are above or below the water to see what kind of fog is applied", "gl_FragColor = vec4(vWorldPosition, 1.0);", "}"].join("\n"),
...
AWater.AOcean.LUTlibraries.OceanHeightBandLibrary = function (parentOceanGrid) {
    ...
    this.L = data.patch_size;
    let windVelocity = new THREE.Vector2(data.wind_velocity.x, data.wind_velocity.y);
    this.L_ = windVelocity.dot(windVelocity) * data.patch_data_size;
    ...
    let maxWaveAmplitutude = .54 * this.L_;
    ...
    noiseVar1.material.uniforms.offset.value = 1;
    ...
    hkRenderer.setVariableDependencies(hkYVar, []);
    ...
    hkYVar.material.uniforms.L.value = 1e3;
    hkYVar.material.uniforms.uTime.value = 500;
    ...
    hkRenderer.setVariableDependencies(hkXVar, []);
    ...
    hkXVar.material.uniforms.L.value = 1e3;
    hkXVar.material.uniforms.uTime.value = 500;
    ...
    hkRenderer.setVariableDependencies(hkZVar, []);
    ...
    hkZVar.material.uniforms.L.value = 1e3;
    hkZVar.material.uniforms.uTime.value = 500;
    ...
    this.hkZBandVars = [];
    let frequencyRadaii = [.05, .01, .002, .0014, 0];
    let bandFrequencyLimits = [1e4, 75e4, 1e7, 3e7, 1e8];
    for (let i = 0; i < this.numLevels; i++) {
        ...
        hkRenderer.setVariableDependencies(this.hkYBandVars[i], [hkYVar]);
        ...
        hkRenderer.setVariableDependencies(this.hkXBandVars[i], [hkXVar]);
        ...
        hkRenderer.setVariableDependencies(this.hkZBandVars[i], [hkZVar]);
    ...
    for (let dimension = 0; dimension < 3; dimension++) {
        ...
        for (let i = 0; i < this.numLevels; i++) {
            ...
            butterflyTextureVars[0].material.uniforms.stageFraction.value = 0;
            ...
            for (let i = 1; i < numPingPongIterations; i++) {
                ...
                butterflyTextureVars[i].material.uniforms.stageFraction.value = i / (numPingPongIterations - 1);
            ...
            for (let i = numPingPongIterations; i < numPingPongIterationsTimes2; i++) {
                ...
                butterflyTextureVars[i].material.uniforms.stageFraction.value = (i - numPingPongIterations) / (numPingPongIterations - 1);
                butterflyTextureVars[i].material.uniforms.twiddleTexture.value = this.twiddleTexture;
                butterflyTextureVars[i].minFilter = THREE.NearestFilter;
                butterflyTextureVars[i].magFilter = THREE.NearestFilter
            }
    ...
    this.tick = function (time, activeTextures) {
        self.hkXVar.material.uniforms.uTime.value = time / 1e3;
        self.hkYVar.material.uniforms.uTime.value = time / 1e3;
        self.hkZVar.material.uniforms.uTime.value = time / 1e3;
...
AWater.AOcean.LUTlibraries.OceanHeightComposer = function (parentOceanGrid) {
    ...
    this.waveHeightComposerRenderer.setVariableDependencies(whcVar, []);
...
AWater.AOcean.Materials.Ocean.waterMaterial = {
    uniforms: {
        ...
        smallNormalMapVelocity: {
            type: "vec2",
            value: new THREE.Vector2
        },
        largeNormalMapVelocity: {
            type: "vec2",
            value: new THREE.Vector2
        },
        isBelowWater: {
        ...
        brightestDirectionalLight: {
            type: "vec3",
            value: new THREE.Vector3(1, 1, 1)
        },
        largeNormalMapStrength: {
            type: "f",
            value: .45
        },
        smallNormalMapStrength: {
            type: "f",
            value: .35
        },
        lightScatteringAmounts: {
            type: "vec3",
            value: new THREE.Vector3(88, 108, 112)
        },
        linearScatteringHeightOffset: {
            type: "f",
            value: 10
        },
        linearScatteringTotalScatteringWaveHeight: {
            type: "f",
            value: 20
        }
    },
...
AWater.AOcean.OceanPatch = function (parentOceanGrid, initialPosition) {
    ...
    this.tick = function (time) {
        ...
        if (self.parentOceanGrid.brightestDirectionalLight) {
            ...
            self.plane.material.uniforms.brightestDirectionalLight.value.set(1, 1, 1)
        }
        self.plane.material.uniforms.t.value = time * .001
    }
};
AWater.AOcean.OceanGrid = function (data, scene, renderer, camera) {
    ...
    const randomAngle1 = Math.random() * 2 * Math.PI;
    const randomAngle2 = Math.random() * 2 * Math.PI;
    this.randomWindVelocities = [2 * Math.cos(randomAngle1), 2 * Math.sin(randomAngle1), 1 * Math.cos(randomAngle2), 1 * Math.sin(randomAngle2)];
    this.raycaster = new THREE.Raycaster(new THREE.Vector3(0, 100, 0), this.downVector);
    this.cameraFrustum = new THREE.Frustum;
    this.brightestDirectionalLight = false;
    this.windVelocity.x = Math.abs(this.data.wind_velocity.x) < .01 ? .01 : this.windVelocity.x;
    this.windVelocity.y = Math.abs(this.data.wind_velocity.y) < .01 ? .01 : this.windVelocity.y;
    const textureLoader = new THREE.TextureLoader;
    let smallNormalMapTexturePromise = new Promise((function (resolve, reject) {
        textureLoader.load(data.small_normal_map, (function (texture) {
            resolve(texture)
        }))
    }));
    ...
    let largeNormalMapTexturePromise = new Promise((function (resolve, reject) {
        textureLoader.load(data.large_normal_map, (function (texture) {
            resolve(texture)
        }))
    }));
    ...
    this.refractionCubeCamera = new THREE.CubeCamera(.1, .5 * this.drawDistance, this.refractionCubeRenderTarget);
    this.scene.add(this.refractionCubeCamera);
    this.depthCubeMapRenderTarget = new THREE.WebGLCubeRenderTarget(512, {
        mapping: THREE.CubeRefractionMapping,
        type: THREE.FloatType
    });
    this.depthCubeCamera = new THREE.CubeCamera(.1, .5 * this.drawDistance, this.depthCubeMapRenderTarget);
    ...
    this.tick = function (time) {
        if (this.brightestDirectionalLight === false) {
            for (let i = 0, numItems = self.scene.children.length; i < numItems; ++i) {
                ...
                if (child.type === "DirectionalLight" && (this.brightestDirectionalLight === false || child.intensity > self.brightestDirectionalLight.intensity)) {
...
AFRAME.registerComponent("ocean-state", {
    ...
    schema: {
        ...
        large_normal_map: {
            type: "string",
            default: "./image-dir/a-water-assets/water-normal-1.png"
        },
        small_normal_map: {
            type: "string",
            default: "./image-dir/a-water-assets/water-normal-2.png"
        },
